...
function moveSelectedOptions(from,to) {
 ...
 for (var i=0; i<from.options.length; i++) {
  ...
  if (o.selected) {
	   ...
	   //release the disabled button
	   if(1==to.options.length){
		 if(true==left_to_right){
			 document.getElementById("move_left_single").disabled=false;
			 document.getElementById("move_left_all").disabled=false;
			 document.getElementById("move_left_single").src="images/nav/b_move_left_single.gif";
			 document.getElementById("move_left_all").src="images/nav/b_move_left_all.gif";
		  }
		 else if(false==left_to_right){
			 document.getElementById("move_right_single").disabled=false;
			 document.getElementById("move_right_all").disabled=false;
			 document.getElementById("move_right_single").src="images/nav/b_move_right_single.gif";
			 document.getElementById("move_right_all").src="images/nav/b_move_right_all.gif";
			}
	   }
   }
  }
 ...
 to.selectedIndex = -1;
 //disabled the button
 if (0 == from.options.length) {
		if (true == left_to_right) {
			document.getElementById("move_right_single").disabled = true;
			document.getElementById("move_right_all").disabled = true;
			document.getElementById("move_right_single").src = "images/nav/b_move_right_single_disabled.gif";
			document.getElementById("move_right_all").src = "images/nav/b_move_right_all_disabled.gif";
		}
		else if (false == left_to_right) {
			document.getElementById("move_left_single").disabled = true;
			document.getElementById("move_left_all").disabled = true;
			document.getElementById("move_left_single").src = "images/nav/b_move_left_single_disabled.gif";
			document.getElementById("move_left_all").src = "images/nav/b_move_left_all_disabled.gif";
		}
	}
}
...
function submitForm() {
   ...
   // ;window.opener ;
   if (document.selector.activeColumns.options.length == 0 && document.selector.required.value !=0){
        alert("At least one column must be selected");
        return false ;
   }

   selectAllOptions(document.selector.activeColumns) ;

    // Set the parent name to be unique.  Otherwise if the user has multiple browsers open
    // and they are all on the same page in our app then submission will go to the wrong parent window
    self.opener.name = new Date().getSeconds();
   document.selector.target = self.opener.name ;
...
function validFileName(f, type) {
   var ext;
   if (f.indexOf(".") != -1) {
      ext = f.substr(f.lastIndexOf(".")).toLowerCase();
      alert("ext " + ext) ;
      if (ext != type) {
        alert("You cannot change the file type");
        return false;
      }
   }
...
function isValidPositiveInt(val, errorStr) {
    if (!errorStr) {
        errorStr = 'Please enter a number > 0';
    }
    if (isNaN(val)) {
        alert(errorStr) ;
        return false;
...
function savePreference (preferenceName, preferenceValue)
{
	...
	oForm.AddField 	= function (sFieldName, sFieldValue)
	{
		...
		this.appendChild(oInputElement);
	}

	with (oForm)
	{
		AddField("preferenceName",  preferenceName);
		AddField("preferenceValue", preferenceValue);
	}
	document.body.appendChild(oForm);
	remoteExecute( oForm, 'sessionPreference.do', null, null, false );
	document.body.removeChild(oForm);
}

//This method is used to mark a page dirty and then notify the page so that menus can be reevaluated.
//PLEASE NOTE: It makes following assumptions:
//   1. pageDirtyFlag is defined and commonly used as a dirty flag object variable name.
//   2. pageEventNotifier is defined and commonly used a PageEventNotifier object variable name.	
function setPageDirty(bSetPageDirty, bNotifyMenus) 
{     
	if ( bSetPageDirty == true && (typeof (pageDirtyFlag) != 'undefined') && pageDirtyFlag != null)
	{
        pageDirtyFlag.setDirty();  // Marks the page as dirty
	}
	if (bNotifyMenus == true && (typeof (pageEventNotifier) != 'undefined') && pageEventNotifier != null)
    {
        pageEventNotifier.pageEventFired ();
    }
	
}
function markPageDirty(bNotifyMenus) 
{     
	setPageDirty(true, bNotifyMenus);
}

function handleKeyDownEventForTextField (oEvent, sourceField, fieldMaxLengthLimit, bMarkPageDirty, bNotifyMenus, bTextCount) 
{     
    var oEvent = maa.event.FixEvent(oEvent);
	if (!oEvent.ctrlKey && !oEvent.shiftKey && !oEvent.altKey)
	{
	   if (bMarkPageDirty != null && bMarkPageDirty == true)
	   {
	      if (!pageDirtyFlag.isDirty)
	      {
	         if ((oEvent.keyCode == 8) ||  //backspace
	         	 (oEvent.keyCode == 32) || //space
	             (oEvent.keyCode == 46) || //delete
	             (oEvent.keyCode >= 48 && oEvent.keyCode <= 57) || //numbers
	             (oEvent.keyCode >= 65 && oEvent.keyCode <= 90) || //chars
	             (oEvent.keyCode >= 96 && oEvent.keyCode <= 107)|| //numberpad
	             (oEvent.keyCode >= 109 && oEvent.keyCode <= 111)||//numberpad
	             (oEvent.keyCode >= 186 && oEvent.keyCode <= 192)||//semi-colon,equal sign,comma,dash,period,forward slash,grave accent
	             (oEvent.keyCode >= 219 && oEvent.keyCode <= 222)||//open bracket,back slash,close braket,single quote          			
	         	 (oEvent.target.nodeName.toLowerCase() == "textarea" && oEvent.keyCode == 13))
	         {
	            markPageDirty (bNotifyMenus);
				if (sourceField && typeof sourceField.fireEvent == "object")
				{
					sourceField.fireEvent("onchange");
				}
				else if (sourceField && typeof sourceField.onchange == "function")
				{
	         ...
	         {
	        	maa.event.CancelEventBubble(oEvent);
	            ///oEvent.returnValue=true;
	         }
	      }
	   }
	   if (bTextCount != null && bTextCount == true)
	   {
	      textCount (sourceField, fieldMaxLengthLimit);
	   }   
	}
}
